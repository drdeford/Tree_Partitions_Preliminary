...
def propose_tree(partition):
    ...
    clusters = recursive_tree_part(sgraph,2,"POP10",.01,2)
    #print(len(clusters))
    flips={}
...
def tree_cut_delta(partition,ST, pop_col, pop_target, epsilon, node_repeats=5):
    ...
    root = random.choice([x for x in ST.nodes() if ST.degree(x)>1]) # probably should be 2 maybe doesn't matter?
    #print(root)
...
def propose_merge2_tree(partition):
    ...
    clusters = recursive_tree_part(sgraph,2,"population",.01,10)
    #print(len(clusters))
    flips={}
    for val in clusters.keys():
        flips[val]=edd[clusters[val]]
...
def propose_bounce_single_cut(partition):
    ...
    clusters = recursive_tree_full(partition,tree,len(partition.parts),"population",.9,2)#was .05 and 2
